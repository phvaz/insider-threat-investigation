...
> **Standards note.** This phase is governed primarily by **ISO/IEC 27037** (acquisition and preservation of digital evidence) and **NIST SP 800-86** (integrating forensic techniques into incident response). The decision to capture volatile memory before the disk — and before shutdown — follows the **order of volatility** defined in **RFC 3227** (Guidelines for Evidence Collection and Archiving). Integrity is established through dual hashing (MD5 + SHA-256) and read-only handling of the source throughout.
